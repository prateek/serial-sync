...
- Wrapping an EPUB 2 attachment upgrades its package to EPUB 3. The NCX becomes an EPUB 3 navigation document with the same entries, author roles and sort names become EPUB 3 refinements, the cover becomes the `cover-image` item, and populated guide links stay. Empty guide sections are removed before validation. Chapter content and the spine are unchanged, so saved reading positions still resolve.
- A wrapped chapter's table of contents lists the post preface first as "Author's note", and a landmarks navigation marks the first story page as body matter so readers can open on the story. Generated chapters and volumes carry the same body-matter landmark.
...
Assembled books/volumes retain member navigation and may add one final author page with biographies, portraits, and descriptive link labels. Assembly removes serial-sync-generated member pages, identified by `serial-sync:about`. Original author back matter and configured post-prefaces survive. Captured originals are unchanged. `preserve` retains its existing byte-preserving/wrapping contract, except that wrapping upgrades an EPUB 2 attachment to EPUB 3.
...
Standalone `epub` chapters are titled by the chapter, not the series: the `dc:title` is "Chapter 430", or "Book Two, Chapter 71" when the author restarts numbering per book, plus any subtitle after the chapter marker in the post title ("Chapter 12: The Fall"). A range reads "Chapters 447-448", a bracketed note stays beside the number ("Chapter 46 (Interlude 1)"), and a post numbered only by part reads "Part 2". A post without a chapter number keeps its title minus a leading series name. The series name lives only in `calibre:series` and `belongs-to-collection`. The full post title is kept as a second `dc:title` refined as `title-type="expanded"`. When the table of contents has exactly one story entry and its label is generic ("Start", "Chapter", "Unknown", the file name), the post title, or starts with the series name, that label becomes the chapter title, because readers show it in their running head. An author's multi-entry table of contents is left alone. `preserve` output keeps the post title and its table of contents.

Attachment titles and creators are retained by default (`identity_source = "embedded"`), except that a chapter's embedded title loses a leading series name ("Harbor - Chapter 9: Salt" becomes "Chapter 9: Salt", with the original kept as the expanded title). If a series has unreliable embedded identity, set `identity_source = "release"` in its metadata: published `epub` chapter copies use the chapter title derived from each source post and the configured canonical author, falling back to the captured creator. This replaces old title/creator entries and their obsolete identity refinements while retaining story resources and unrelated metadata. Author profiles enrich embedded metadata and assembled author pages; they alone do not override embedded creators. Books inherit the series policy and can explicitly select `"embedded"` again. These are the only accepted nonempty values. Volumes keep their assembled volume title.
