...
- A standalone `epub` chapter's book title is its own chapter title, never the series name (changed 2026-09-26 from "the release title, trimmed", because readers show the book title and table-of-contents label in their running head, and "The Sixth School. Book Two. Chapter 071." repeated the series beside the series name). It comes from the detected numbering: "Book Two, Chapter 71" when numbering restarts per book, "Chapter 430" otherwise, followed by any subtitle the release gives after its chapter marker. A release without a chapter number keeps its title minus a leading series name. The release title stays in the package as a `title-type="expanded"` title, and the chapter's single table-of-contents entry carries the chapter title. `preserve` output keeps the release title. The series title is the series config title and appears only in series metadata.
