...
- Derive these positions from declared ranges, never the number of chapters currently captured. If earlier book ranges or the starting position are unknown, preview reports the missing mapping and chapter output omits the ambiguous scalar position. Do not give two books' chapter 1 the same position or invent decimal encodings. Volume publication requires an unambiguous series position.
- A volume uses its first covered chapter's series position, so it sorts alongside open singles. Its title identifies the author book or the fixed volume number and chapter range; the table of contents uses original chapter titles. An unnumbered release has no position and retains its original publish date.
...
- The repo has no domain glossary or ADRs. Terms here follow the config and code: source, series, series input, release, release role, artifact, publisher, run.
