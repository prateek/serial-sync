...
- standalone `epub` chapters are titled by the chapter ("Chapter 430", "Book Two, Chapter 71", plus any post subtitle) with the post title kept as the expanded title; check detected numbering in preview when a running head looks wrong, since the title follows the same sequence and series index
...
Prefer explicit curated values, then suitable original embedded fields, then directly linked creator/collection fallbacks. Never equate portraits, campaign banners, and book covers. Review ambiguous web identities offline. Keep original captured attachments unchanged. `epub` produces enriched standalone copies without generated About pages. Assembled books/volumes may carry one author page; book labels and attachment roles alone do not establish completeness. Preserve original author back matter and configured post-prefaces. Source URLs and author biographies remain embedded metadata. `preserve` retains its byte-preserving/wrapping contract, except that wrapping upgrades an EPUB 2 attachment to EPUB 3.

When embedded titles or creators conflict with reviewed source posts, select series/book `metadata.identity_source = "release"` and verify each resulting chapter title and canonical author in the rebuilt EPUBs. The default `"embedded"` policy preserves original identity; author profiles alone do not change it. Books can override the series policy. See the metadata reference for inheritance and replacement details.

A metadata refresh must not silently replace an existing reading copy. Inspect offline preview, then an explicit rebuild preview for already published files. Review multi-chapter attachment navigation, absence of generated chapter back matter, and at most one final author page in assembled volumes. Older chapter About pages require an explicit rebuild even without config edits; library reconciliation and redownloading offline reader copies are separate checks. Pending deliveries must keep their saved bytes. For BookOrbit, use a `drop` publisher into a watched library folder; it hands each release off once and holds later revisions, so a rebuild does not replace BookOrbit books.

Keep user reading interest outside series classification and EPUB metadata. Readers silence a series by unfollowing it in BookOrbit; fetching and publishing continue. BookOrbit owns reading status and locators. Snapshot existing state and exact chapter coverage before bulk read-through changes, and leave future arrivals unread. See the [BookOrbit library guide](../../integrations/bookorbit/README.md).
